...
namespace pxsim {
    ...
    export class BitBotBoard extends BaseBoard {
        ...
        public setDrive(left: number, right: number): void {
            ...
            this.state.heading += (this.state.right - this.state.left) * 0.35;
    ...
    export let board: BitBotBoard;

    export function init(): void {
        board = new BitBotBoard();
    }
    ...
    export function _move(motor: number, direction: number, speed: number): void {
        const dir = direction === 0 ? 1 : -1;
        const value = Math.max(0, Math.min(100, speed)) * dir;
        if (motor === 0 || motor === 2) board.state.left = value;
        if (motor === 1 || motor === 2) board.state.right = value;
    }
}
